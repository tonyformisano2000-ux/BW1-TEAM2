...
results(mostraDomande());
const results = (voto) => {
  const percentualeRisposteCorrette = (voto / domanda.length) * 100;
  console.log(percentualeRisposteCorrette + "%");
  const nEserciziCorretti = voto + ` / ${domanda.length} questions`;
  ...
  const nEserciziSbagliati =
    domanda.length - voto + ` / ${domanda.length} questions`;
